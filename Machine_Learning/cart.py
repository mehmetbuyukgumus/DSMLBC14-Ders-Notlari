...
# AUC için y_prob:
y_prob = cart_model.predict_proba(X)
...
cv_results = cross_validate(cart_model, X, y, cv=5, scoring=['roc_auc', 'accuracy', "f1"])
...
## Burda bir soru: Test verisi ile train verisi yapı olarak farklı veriler değil. O sebeple 2 gözlem gördüğünde model
# bölme işlemi yapıyorsa benzer hataları train verisinde de almamız gerekmiyor mu?
...
# Not: Ben bu noktada GridSearchCV ile "cross_validate"i kartıştırıyordum. Şimdi daha iyi anladım. "cross_validate"
# model performansı ölçümü için kullanılıyor. GirdSearchCV ise heperparametre optimizasyonu için.
# Not: GridSearchCV'deki n_jobs=-1 işlemciyi en performanslı şekilde kullanmak için
# Not: verbose=1 veya True ve verbose=2 ifadesi ise raporlama şekliyle ilgili
...
# 5. Final Model
################################################
cart_final_model = DecisionTreeClassifier(**cart_best_grid.best_params_, random_state=17)
cart_final_model.fit(X, y)

cart_final_model = cart_model.set_params(**cart_best_grid.best_params_)

cv_results = cross_validate(cart_final_model, X, y, cv=5, scoring=['roc_auc', 'accuracy', "f1"])
cv_results["test_accuracy"].mean()
cv_results["test_roc_auc"].mean()
cv_results["test_f1"].mean()
...
train_score, test_score = validation_curve(cart_final_model, X, y, param_name="max_depth", param_range=(1, 11),
...
# 12. Saving and Loading Model
################################################
joblib.dump(cart_final_model, "cart_final_model.pkl")
cart_model_from_disc = joblib.load("cart_final_model.pkl")
x = [12, 13, 20, 23, 4, 55, 12, 7]
